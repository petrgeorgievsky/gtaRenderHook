...
int32_t RsCameraBeginUpdate( RwCamera *camera )
{
    ...
    return reinterpret_cast<decltype( &RsCameraBeginUpdate )>( f_ptr )( camera );
}
...
RwCamera *rh::RwCameraBeginUpdate( RwCamera *camera )
{
    ...
    return reinterpret_cast<decltype( &rh::RwCameraBeginUpdate )>( f_ptr )( camera );
}

RwCamera *rh::RwCameraEndUpdate( RwCamera *camera )
{
    static INT_PTR f_ptr = 0x7EE180;
    return reinterpret_cast<decltype( &rh::RwCameraEndUpdate )>( f_ptr )( camera );
}

RwCamera *rh::RwCameraClear( RwCamera *camera, RwRGBA *color, RwInt32 clearMode )
{
    static INT_PTR f_ptr = 0x7EE340;
    return reinterpret_cast<decltype( &rh::RwCameraClear )>( f_ptr )( camera, color, clearMode );
}
