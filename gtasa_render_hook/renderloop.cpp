...
static GBufferPipeline *g_pGBufferPipeline = nullptr;
static GBufferPass *g_pGBufferPass = nullptr;
...
void RenderLoop::Init()
{
    ...
    rh::rw::engine::RwRenderEngine::RegisterOutputResizeCallback( []( uint32_t w, uint32_t h ) {
        if ( g_pGBufferPass )
            g_pGBufferPass->UpdateSize( w, h );
    } );
...
void RenderLoop::RenderOpaque()
{
    ...
    context->UpdateBuffer( gBaseConstantBuffer,
    ...
    for ( uint32_t i = 0; i < CRenderer::ms_nNoOfVisibleEntities; i++ ) {
        ...
        RwObject *rw_obj = *reinterpret_cast<RwObject **>( reinterpret_cast<INT_PTR>( entity_ptr )
                                                           + 0x18 );
        if ( rw_obj->type == 1 ) {
            auto atomic = reinterpret_cast<RpAtomic *>( rw_obj );
            geometry_interface_35.Init( atomic->geometry );
            if ( rh::rw::engine::RwRHInstanceAtomic( atomic, &geometry_interface_35 )
            ...
            rh::rw::engine::DrawAtomic( atomic, &geometry_interface_35, context, g_pGBufferPipeline );
        }
    }

    g_pGBufferPass->EndFrame( context );
    void *frameBufferInternal = rh::rw::engine::GetInternalRaster( Scene.m_pRwCamera->frameBuffer );
    rh::engine::g_pRHRenderer->BindImageBuffers( rh::engine::ImageBindType::RenderTarget,
