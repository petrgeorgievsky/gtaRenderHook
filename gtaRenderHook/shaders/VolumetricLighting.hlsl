...
float3 ComputeSunRaysWithLighting(float2 vTexCoord, float2 vPosition, float3 LightDir, float3 ViewPos, float3 WorldPos, float Length)
{
        const int SunRaySampleCount = SUNLIGHT_RM_STEPS;
    ...
    //float Jitter =  frac(frac(dot(vPosition.xy, float2(0.06711056,0.00583715))) * 52.9829189); //interleaved gradient noise
        ...
        for(int i = 0; i < SunRaySampleCount; i++)
        {
                float3 CurrentRay = SampleShadowCascadesUnfiltered(txShadow, samShadow, samLinear, CurrentPos, StepLength * (i + Jitter));
...
float4 VolumetricSunlightPS(PS_QUAD_IN i) : SV_TARGET
{
    ...
    // Directions calculation maybe we should introduce macroses to encapsulate them
        ...
        float3 SunRays = ComputeSunRaysWithLighting(i.vTexCoord.xy, i.vPosition.xy, LightDir, ViewPos, WorldPos, min(length(WorldPos - ViewPos), RaymarchingDistance));
