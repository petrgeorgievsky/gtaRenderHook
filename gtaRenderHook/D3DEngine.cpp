...
bool CRwD3DEngine::Open( HWND *pHWND )
{
    ...
    for ( errorCode = FAILED( m_d3d9->GetDeviceCaps(
              m_adapterIndex, m_deviceType, &m_deviceCaps ) );
          ( m_adapterIndex < AdapterCount ) && errorCode; ++m_adapterIndex )
        ;
    if ( errorCode )
    {
...
void CRwD3DEngine::m_setPresentParameters(
    const D3DDISPLAYMODE &CurrentDisplayMode )
{
...
bool CRwD3DEngine::CameraClear( RwCamera *pOut, RwRGBA *pInOut, RwInt32 nI )
{
    ...
    RECT          rect;
    RwRaster *    fbParent = pOut->frameBuffer->parent, *zBuffer;
    ...
    if ( r->swapChain )
    {
        ...
        {
            if ( rect.right != fbParent->originalWidth ||
                 rect.bottom != fbParent->originalHeight )
            {
            ...
            else
            {
                ...
                if ( zBuffer && zBuffer == zBuffer->parent &&
                     ( rect.right != zBuffer->width ||
                       rect.bottom != zBuffer->height ) )
                {
                    RwRasterDestroy( pOut->zBuffer );
                    pOut->zBuffer = nullptr;
                    if ( zBuffer = RwRasterCreate( rect.right, rect.bottom, 0,
                                                   rwRASTERTYPEZBUFFER ) )
                        pOut->zBuffer = zBuffer;
                }
            }
            m_setRenderTarget( 0, r->surface );
            m_setDepthStencilSurface(
                zBuffer ? GetD3D9Raster( zBuffer->parent )->surface : nullptr );
            D3DVIEWPORT9 vp{
                0, 0, pOut->frameBuffer->width, pOut->frameBuffer->height,
                0, 1 };
            pD3DDevice->SetViewport( &vp );
            pD3DDevice->Clear( NULL, nullptr, nI,
                               D3DCOLOR_RGBA( pInOut->red, pInOut->green,
                                              pInOut->blue, pInOut->alpha ),
                               0, NULL );
            return true;
        }
    }
    ...
    if ( rect.right == 0 || rect.bottom == 0 || IsIconic( m_windowHandle ) )
        return false;
    if ( rect.right == m_displayMode.Width &&
         rect.bottom == m_displayMode.Height )
    {
        if ( fbParent->cType != rwRASTERTYPECAMERATEXTURE )
        {
            ...
            D3DVIEWPORT9 vp{ pOut->frameBuffer->nOffsetX,
                             pOut->frameBuffer->nOffsetY,
                             pOut->frameBuffer->width,
                             pOut->frameBuffer->height,
                             0,
                             1 };
            pD3DDevice->SetViewport( &vp );
            pD3DDevice->Clear( NULL, nullptr, nI,
...
bool CRwD3DEngine::CameraBeginUpdate( RwCamera *camera )
{
    ...
    RwMatrixInvert( &RwD3D9D3D9ViewTransform,
    ...
    RwD3D9D3D9ProjTransform.at.x =
        camera->viewOffset.x * camera->recipViewWindow.x;
    RwD3D9D3D9ProjTransform.at.y =
        camera->viewOffset.y * camera->recipViewWindow.y;
    RwD3D9D3D9ProjTransform.pos.x =
        -( camera->viewOffset.x * camera->recipViewWindow.x );
    RwD3D9D3D9ProjTransform.pos.y =
        -( camera->viewOffset.y * camera->recipViewWindow.y );
    if ( camera->projectionType == rwPARALLEL )
    {
        RwD3D9D3D9ProjTransform.at.z =
    ...
    RwD3D9D3D9ProjTransform.pos.z =
        -( RwD3D9D3D9ProjTransform.at.z * camera->nearPlane );
    ...
    if ( !fbParent->swapChain )
    {
        ...
        if ( rect.right != m_displayMode.Width ||
             rect.bottom != m_displayMode.Height )
        {
            m_pDebug->printMsg( "CamBegUpd: Window don't have same size as "
                                "display mode! Rect r:" +
                                to_string( rect.right ) +
                                " b:" + to_string( rect.bottom ) );
            ReleaseVideoMemory();
            if ( pD3DDevice->TestCooperativeLevel() >= 0 &&
            ...
            if ( pD3DDevice->Reset( &m_presentParams ) >= 0 )
            {
                ...
                if ( m_videoMemoryRasterListRestore() )
                {
                    ...
                    m_pDebug->printMsg(
                        "CamBegUpd: Device restore buffers call!" );
        ...
        if ( camera->frameBuffer->parent->cType != rwRASTERTYPECAMERATEXTURE )
        {
            ...
            D3DVIEWPORT9 vp{ camera->frameBuffer->nOffsetX,
                             camera->frameBuffer->nOffsetY,
...
bool CRwD3DEngine::RasterDestroy( RwRaster *raster )
{
    ...
    else
    {
        ...
        case rwRASTERTYPETEXTURE:
            ...
            {
                if ( d3dRaster->format >= D3DFMT_D16_LOCKABLE &&
                     d3dRaster->format <= D3DFMT_D16 )
                    m_videoMemoryRasterListRemove( raster );
            }
        ...
        case rwRASTERTYPEZBUFFER:
            if ( raster->cFlags < 0 )
                return true;
            m_videoMemoryRasterListRemove( raster );
            if ( d3dRaster->surface != m_depthStencilSurface &&
                 d3dRaster->surface )
...
bool CRwD3DEngine::RasterLock( RwRaster *raster, UINT flags, void **data )
{
    ...
    case rwRASTERTYPETEXTURE:
        ...
        if ( d3dTopLevelRaster->texture->GetSurfaceLevel(
                 flags >> 8, &d3dRaster->lockedSurface ) < 0 )
            return false;
        if ( topLevelRaster == raster )
        {
            if ( d3dRaster->lockedSurface->LockRect( &d3dRaster->lockedRect,
                                                     NULL, d3dLockFlags ) < 0 )
                return false;
        }
        else
...
bool CRwD3DEngine::RasterUnlock( RwRaster *raster )
{
    ...
    case rwRASTERTYPETEXTURE:
        ...
        if ( !( raster->privateFlags & rwRASTERPIXELLOCKEDWRITE ) ||
             !( raster->cFormat & 0x10 ) || d3dRaster->lockFlags ||
             d3dRaster->textureFlags & 0xF )
        {
...
bool CRwD3DEngine::RasterShowRaster( RwRaster *raster, UINT flags )
{
    ...
    {
        if (m_presentParams.PresentationInterval ==
    D3DPRESENT_INTERVAL_IMMEDIATE) { m_presentParams.PresentationInterval =
    D3DPRESENT_INTERVAL_ONE; m_resetDevice();
        }
    }
    else
    {
    ...
    D3DPRESENT_INTERVAL_IMMEDIATE; m_resetDevice();
        }
    }*/
    HRESULT hr;
    if ( GetD3D9Raster( raster )->swapChain )
        hr = GetD3D9Raster( raster )->swapChain->Present( NULL, NULL, NULL,
                                                          NULL, NULL );
    else
...
bool CRwD3DEngine::NativeTextureRead( RwStream *stream, RwTexture **tex )
{
    ...
    RasterFormat  rasterInfo;
    unsigned int  lengthOut, versionOut, numLevels;
    unsigned char savedFormat;
    RwRaster *    raster;
    RwTexture *   texture;
    RwD3D9Raster *d3dRaster;

    if ( !RwStreamFindChunk( stream, rwID_STRUCT, &lengthOut, &versionOut ) ||
         versionOut < 0x34000 || versionOut > rwLIBRARYVERSION36003 ||
         RwStreamRead( stream, &textureInfo, sizeof( TextureFormat ) ) !=
             sizeof( TextureFormat ) ||
         textureInfo.platformId != rwID_PCD3D9 ||
         RwStreamRead( stream, &rasterInfo, sizeof( RasterFormat ) ) !=
             sizeof( RasterFormat ) )
        return false;
    if ( rasterInfo.compressed )
    {
        if ( m_checkValidTextureFormat( rasterInfo.d3dFormat ) )
        {
            raster = RwRasterCreate(
                rasterInfo.width, rasterInfo.height, rasterInfo.depth,
        ...
        numLevels =
            ( raster->cFormat & FORMATMIPMAP ) ? rasterInfo.numLevels : 1;
        if ( !rasterInfo.cubeTexture )
        {
            // if ((rasterInfo.rasterFormat & rwRASTERFORMATMIPMAP) &&
            // (rasterInfo.rasterFormat & rwRASTERFORMATAUTOMIPMAP) &&
            //	rwD3D9CheckAutoMipmapGenCubeTextureFormat(rasterInfo.d3dFormat))
            d3dRaster->textureFlags |= 1;
            if ( pD3DDevice->CreateTexture(
                     raster->width, raster->height, numLevels,
                     D3DUSAGE_AUTOGENMIPMAP, rasterInfo.d3dFormat,
    ...
    else
    {
        if ( !rasterInfo.cubeTexture )
        {
            ...
            (rasterInfo.rasterFormat & rwRASTERFORMATAUTOMIPMAP))
            {
                raster = RwD3D9RasterCreate(rasterInfo.width, rasterInfo.height,
            rasterInfo.d3dFormat, rasterInfo.rasterType |
                    (rasterInfo.rasterFormat & (rwRASTERFORMATMIPMAP |
            rwRASTERFORMATAUTOMIPMAP))); if (!raster) return false;
            }*/
            // else
            //{
            raster = RwRasterCreate(
            ...
            if ( !raster )
                return false;
            d3dRaster = GetD3D9Raster( raster );
            // raster->d3dRaster.hasCompression = 0; // �����??
            //}
        }
    }
    raster->cFlags ^= rwRASTERDONTALLOCATE;
    ...
    /*if (rasterInfo.autoMipMaps && !raster->d3dRaster.autoMipMapping ||
    rasterInfo.rasterFormat != ((raster->cFormat) << 8) || rasterInfo.d3dFormat
    != raster->d3dRaster.format || rasterInfo.width != raster->width ||
    rasterInfo.height != raster->height)
    {
        RwRasterDestroy(raster);
        return false;
    }*/
    ...
    {
        if (RwStreamRead(stream, RwRasterLockPalette(raster, rwRASTERLOCKWRITE),
    128) != 128)
        {
        ...
        for ( int j = 0; j < 1; j++ )
        {
            if ( RwStreamRead( stream, &lengthOut, 4 ) == 4 )
            {
                if ( RwStreamRead( stream,
                                   RwRasterLock( raster, j, rwRASTERLOCKWRITE ),
                                   lengthOut ) == lengthOut )
                {
...
bool CRwD3DEngine::CreateVertexDeclaration( D3DVERTEXELEMENT9 *elements,
                                            IDirect3DVertexDeclaration9 **decl )
{
    ...
    if ( m_numVertexDeclarations > 0 )
    {
        ...
        {
            if ( m_vertexDeclarations[i].elements_count == elementCount &&
                 m_vertexDeclarations[i].decl == *decl )
            {
    ...
    if ( m_numVertexDeclarations >= m_maxVertexDeclarations )
    {
        ...
        if ( m_vertexDeclarations )
            m_vertexDeclarations = (RwD3D9VertexDecl *)realloc(
                m_vertexDeclarations, 12 * m_maxVertexDeclarations );
        else
            m_vertexDeclarations =
                (RwD3D9VertexDecl *)malloc( 12 * m_maxVertexDeclarations );
    }
    ...
    else
    {
        ...
        m_vertexDeclarations[m_numVertexDeclarations].elements_count =
            elementCount;
        m_vertexDeclarations[m_numVertexDeclarations].elements =
            (D3DVERTEXELEMENT9 *)malloc( elementCount *
                                         sizeof( D3DVERTEXELEMENT9 ) );
        memcpy( m_vertexDeclarations[m_numVertexDeclarations].elements,
                elements, elementCount * sizeof( D3DVERTEXELEMENT9 ) );
...
bool CRwD3DEngine::CreatePixelShader( const DWORD *           pFunction,
                                      IDirect3DPixelShader9 **ppShader )
{
    return SUCCEEDED( pD3DDevice->CreatePixelShader( pFunction, ppShader ) );
}

bool CRwD3DEngine::DynamicVertexBufferCreate(
    UINT size, IDirect3DVertexBuffer9 **vertexbuffer )
{

    if ( m_dynamicVertexBufferList == nullptr )
        m_dynamicVertexBufferList = new list<dVB>;
    if ( pD3DDevice->CreateVertexBuffer( size, 520, 0, D3DPOOL_DEFAULT,
                                         vertexbuffer, 0 ) )
        return false;
    else
    {
...
bool CRwD3DEngine::DynamicVertexBufferLock(
    RwUInt32 vertexSize, RwUInt32 numVertex,
    IDirect3DVertexBuffer9 **vertexBufferOut, void **vertexDataOut,
    RwUInt32 *baseIndexOut )
{
    int lockSize = numVertex * vertexSize, lockOffset;
    if ( m_deviceCaps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT )
    {
        lockOffset = m_dynamicVertexBufferMgrs[m_currentDVBMgr].offset;
        if ( lockOffset + lockSize >
             m_dynamicVertexBufferMgrs[m_currentDVBMgr].size )
        {
    ...
    if ( lockSize > m_dynamicVertexBufferMgrs[m_currentDVBMgr].size )
    {
        ...
        if (dvbList)
        {
            while (dvbList->pParent!=nullptr && dvbList->pVB !=
        m_dynamicVertexBufferMgrs[m_currentDVBMgr].vertexbuffer)
            {
    ...
    {
        if ( FAILED(
                 m_dynamicVertexBufferMgrs[m_currentDVBMgr].vertexbuffer->Lock(
                     lockOffset, lockSize, vertexDataOut,
                     D3DLOCK_NOSYSLOCK | D3DLOCK_NOOVERWRITE ) ) )
            return false;
        m_dynamicVertexBufferMgrs[m_currentDVBMgr].offset =
            lockOffset + lockSize;
...
void CRwD3DEngine::SetMultiSamplingLevels( int value )
{
    ...
    if ( value >= 2 )
    {
        for ( m_selectedMultisamplingLevels; m_selectedMultisamplingLevels >= 2;
              m_selectedMultisamplingLevels-- )
            if ( m_d3d9->CheckDeviceMultiSampleType(
                     m_adapterIndex, m_deviceType, m_displayMode.Format,
                     m_windowed,
                     (D3DMULTISAMPLE_TYPE)m_selectedMultisamplingLevels,
                     0 ) >= 0 )
                break;

        m_selectedMultisamplingLevelsNonMask =
            value * m_maxMultisamplingLevelsNonMask / m_maxMultisamplingLevels;
    }
    else
    {
...
bool createPixelShader( const DWORD *           pFunction,
                        IDirect3DPixelShader9 **ppShader )
{
    return static_cast<CRwD3DEngine *>( g_pRwCustomEngine )
...
bool CRwD3DEngine::Im2DRenderPrimitive( RwPrimitiveType primType,
                                        ...
                                        RwInt32         numVertices )
{
    ...
    switch ( primType )
    {
    case rwPRIMTYPELINELIST: primCount = numVertices / 2; break;
    case rwPRIMTYPEPOLYLINE: primCount = numVertices - 1; break;
    case rwPRIMTYPETRILIST: primCount = numVertices / 3; break;
    case rwPRIMTYPETRISTRIP:
    case rwPRIMTYPETRIFAN: primCount = numVertices - 2; break;
    ...
    if ( m_useOwnVertexBuffer )
    {
        if ( ( m_currentBaseIndex + numVertices ) <= 9362 )
            m_currentVertexBuffer->Lock(
                sizeof( RwIm2DVertex ) * m_currentBaseIndex,
                sizeof( RwIm2DVertex ) * numVertices, (void **)&vertexData,
                D3DLOCK_NOSYSLOCK | D3DLOCK_NOOVERWRITE );
        else
        {
            m_currentBaseIndex = 0;
            m_currentVertexBuffer->Lock(
                0, sizeof( RwIm2DVertex ) * numVertices, &vertexData,
                D3DLOCK_NOSYSLOCK | D3DLOCK_DISCARD );
        }
    }
    else
    {
        DynamicVertexBufferLock( 28, numVertices, &m_currentVertexBuffer,
                                 &vertexData, &m_currentBaseIndex );
    }
    ...
    else
    {
        if ( raster->nOffsetX == 0 && raster->nOffsetY == 0 )
            memcpy( vertexData, vertices,
                    4 * ( ( sizeof( RwIm2DVertex ) * numVertices ) >> 2 ) );
        else
            m_pDebug->printError( "raster has offsets!" );
        m_currentVertexBuffer->Unlock();
        m_im2DRenderFlush();
        m_pDebug->printMsg( "Base index:" + to_string( m_currentBaseIndex ) );
        if ( FAILED( pD3DDevice->DrawPrimitive(
                 m_primConvTable[primType], m_currentBaseIndex, primCount ) ) )
        {
            m_pDebug->printError( "failed to draw!" );
            return false;
        }
...
void CRwD3DEngine::m_clearCacheMatrix()
{
    ...
    {
        MatrixFreeList = RwFreeListCreate(
            64, 15, 16, rwMEMHINTDUR_EVENT | rwID_DRIVERMODULE );
    ...
    u_C98070 = (RwMatrix *)_rwFreeListAlloc(
        MatrixFreeList, rwMEMHINTDUR_EVENT | rwID_DRIVERMODULE );
...
{
    if ( m_displayModes )
        m_displayModes = (RwDisplayMode *)realloc(
            m_displayModes, 20 * ( m_numAdapterModes + 1 ) );
    else
        m_displayModes =
            (RwDisplayMode *)malloc( 20 * ( m_numAdapterModes + 1 ) );
    ...
    case D3DFMT_A2R10G10B10:
        ...
        m_numDisplayModes       = 1;
        break;
    default: m_numDisplayModes = 0; break;
    }
    int adapterCount = 0;
    for ( auto fmt : m_aBackBufferFormat )
    {
        ...
        if ( AdapterCount > 0 )
        {
            ...
            {
                m_d3d9->EnumAdapterModes( m_adapterIndex, fmt, i,
                                          &m_displayModes[adapterCount] );
                bool bTest = true;
                for ( int j = 1; j < adapterCount - 1; j++ )
                {
                    if ( m_displayModes[j].Width ==
                             m_displayModes[adapterCount].Width &&
                         m_displayModes[j].Height ==
                             m_displayModes[adapterCount].Height &&
                         m_displayModes[j].Format ==
                             m_displayModes[adapterCount].Format )
                        bTest = false;
                }
                if ( bTest )
    ...
    if ( adapterCount < m_numAdapterModes + 1 )
        m_displayModes =
            (RwDisplayMode *)realloc( m_displayModes, 20 * adapterCount );
}
...
void CRwD3DEngine::m_calculateMaxMSLevels()
{
    ...
    m_selectedMultisamplingLevelsNonMask = 1;
    for ( int i = D3DMULTISAMPLE_2_SAMPLES; i <= D3DMULTISAMPLE_16_SAMPLES;
          i++ )
        if ( m_d3d9->CheckDeviceMultiSampleType(
                 m_adapterIndex, m_deviceType, m_displayMode.Format, m_windowed,
                 static_cast<D3DMULTISAMPLE_TYPE>( i ), 0 ) >= 0 )
            m_maxMultisamplingLevels = i;
    int tmp;
    if ( m_d3d9->CheckDeviceMultiSampleType(
             m_adapterIndex, m_deviceType, m_displayMode.Format, m_windowed,
             D3DMULTISAMPLE_NONMASKABLE, (DWORD *)&tmp ) >= 0 )
        m_maxMultisamplingLevels =
            max( ( m_selectedMultisamplingLevelsNonMask = tmp + 1 ),
                 m_maxMultisamplingLevels );
}

void CRwD3DEngine::m_rasterOpen()
{
    VideoMemoryRastersFreeList =
        RwFreeListCreate( 8, 127, 4, rwMEMHINTDUR_EVENT | rwID_DRIVERMODULE );
}
...
void CRwD3DEngine::m_im2DRenderOpen()
{
    ...
    if ( !( m_deviceCaps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT ) &&
         ...
         pD3DDevice->CreateVertexBuffer(
             0x40000,
             D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY | D3DUSAGE_DONOTCLIP, 0,
             D3DPOOL_DEFAULT, &m_currentVertexBuffer, 0 ) == D3D_OK )
    {
        m_useOwnVertexBuffer = true;
        m_currentBaseIndex   = 0;
    }
    IB2DOffset = 0;
    if ( pD3DDevice->CreateIndexBuffer(
             20000, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY | D3DUSAGE_DONOTCLIP,
             D3DFMT_INDEX16, D3DPOOL_DEFAULT, &IndexBuffer2D, 0 ) != D3D_OK )
    {
        if ( m_vertexDeclIm2D )
        {
...
{
    if ( pD3DDevice->CreateIndexBuffer(
             0x1FFFE, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFMT_INDEX16,
             D3DPOOL_DEFAULT, &IndexBuffer3D, nullptr ) != D3D_OK )
    {
...
void CRwD3DEngine::m_vertexBufferManagerOpen()
{
    ...
    StrideFreeList =
        RwFreeListCreate( 16, 16, 4, rwMEMHINTDUR_GLOBAL | rwID_DRIVERMODULE );
    ...
    for ( int i = 0; i < 4; i++ )
    {
        ...
        if ( m_dynamicVertexBufferMgrs[i].vertexbuffer )
        {
            if ( m_dynamicVertexBufferList )
            {
                ...
                /*while (pEntry->pVB !=
                m_dynamicVertexBufferMgrs[i].vertexbuffer)
                {
        ...
        while ( vmrList->pParent )
        {
            ...
            if ( raster->parent == raster )
            {
                    ...
                    if ( !( raster->cFormat & 0x60 ) )
                    {
                        if ( d3draster->format >= D3DFMT_D16_LOCKABLE &&
                            ...
                            if ( pD3DDevice->CreateTexture(
                                     ...
                                     D3DUSAGE_DEPTHSTENCIL, d3draster->format,
                                     D3DPOOL_DEFAULT, &d3draster->texture,
                                     0 ) < 0 )
                                return false;
                    }
                    break;
                ...
                case rwRASTERTYPEZBUFFER:
                    ...
                    {
                        d3draster->format =
                            m_checkValidZBufferFormat( D3DFMT_D15S1 ) != 0
                                ? D3DFMT_D15S1
                                : D3DFMT_D16;
                    }
                    else if ( m_ZBufferDepth == 32 )
                    {
                        if ( m_checkValidZBufferFormat( D3DFMT_D24S8 ) )
                            ...
                            d3draster->format =
                                m_checkValidZBufferFormat( D3DFMT_D32 )
                                    ? D3DFMT_D32
                                    : D3DFMT_D24X8;
                    }
                    if ( pD3DDevice->CreateDepthStencilSurface(
                             raster->width, raster->height, d3draster->format,
                             m_presentParams.MultiSampleType,
                             m_presentParams.MultiSampleQuality, 0,
                             &d3draster->surface, 0 ) < 0 )
                        return false;
                    break;
                case rwRASTERTYPECAMERA:
                    if ( d3draster->swapChain )
                    {
                        ...
                        if ( SUCCEEDED( pD3DDevice->CreateAdditionalSwapChain(
                                 &tmp, &d3draster->swapChain ) ) )
                        {
                            if ( d3draster->swapChain->GetBackBuffer(
                    ...
                    else
                    {
                        ...
                        raster->depth   = 32;
                        raster->cFormat = m_getRasterFormat(
    ...
    if ( m_deviceCaps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT )
    {
        if ( m_dynamicVertexBufferList )
        {
            for ( dVB &VB : *m_dynamicVertexBufferList )
            {
                if ( VB.vb == nullptr )
                {
                    if ( FAILED( pD3DDevice->CreateVertexBuffer(
                             VB.size, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY,
                             NULL, D3DPOOL_DEFAULT, &VB.vb, nullptr ) ) )
                        return;
                }
            }
            ...
            while (dynVBlist->pParent)
            {
                ...
                {
                    if (FAILED(pD3DDevice->CreateVertexBuffer(dynVBlist->length,
            D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, NULL, D3DPOOL_DEFAULT,
            &dynVBlist->pVB, nullptr))) return; *dynVBlist->ppVB =
            *&dynVBlist->pVB;
                }
...
{
    RwVideoMemoryRaster *vmRaster = (RwVideoMemoryRaster *)_rwFreeListAlloc(
        VideoMemoryRastersFreeList, 0x30411 );
    vmRaster->pCurrent = r;
...
void CRwD3DEngine::m_dynamicVertexBufferManagerCreate()
{
    ...
    if ( m_deviceCaps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT )
    {
        for ( int i = 0; i < 4; i++ )
        {
            ...
            if ( !DynamicVertexBufferCreate(
                     0x40000u, &m_dynamicVertexBufferMgrs[i].vertexbuffer ) )
                return;
        }
    }
    else
    {
        ...
        if ( DynamicVertexBufferCreate(
                 0x40000u, &m_dynamicVertexBufferMgrs[0].vertexbuffer ) )
        {
...
bool CRwD3DEngine::m_checkTextureFormat( RwRaster *raster, UINT flags )
{
    ...
    case rwRASTERTYPETEXTURE:
        if ( rasterPixelFmt )
        {
            ...
            if ( flags & rwRASTERFORMATPAL8 )
            {
                if ( !m_checkValidTextureFormat( D3DFMT_P8 ) ||
                     rasterPixelFmt != rwRASTERFORMAT888 &&
                         ( rasterPixelFmt != rwRASTERFORMAT8888 ||
                           !( m_deviceCaps.TextureCaps &
            ...
            {
                D3DFORMAT d3dRasterFmt =
                    m_rasterConvertTable[( rasterFmt >> 8 ) & 0xF].fmt;
                if ( d3dRasterFmt < 70 || d3dRasterFmt > 80 )
                {
                    if ( !m_checkValidTextureFormat(
                             m_rasterConvertTable[( rasterFmt >> 8 ) & 0xF]
                                 .fmt ) )
                        return false;
                    raster->depth =
                        m_rasterConvertTable[( rasterFmt >> 8 ) & 0xF].depth;
                }
                else
                {
                    if ( !m_checkValidZBufferTextureFormat(
                             m_rasterConvertTable[( rasterFmt >> 8 ) & 0xF]
                                 .fmt ) )
                        return false;
                    raster->depth =
        ...
        if ( !( rasterFmt & rwRASTERFORMATPAL8 ) &&
                 ( flags &
                   ( rwRASTERFORMATAUTOMIPMAP | rwRASTERFORMATMIPMAP ) ) ==
                     ( rwRASTERFORMATAUTOMIPMAP | rwRASTERFORMATMIPMAP ) ||
             m_checkAutoMipmapGenTextureFormat(
                 m_rasterConvertTable[( rasterFmt >> 8 ) & 0xF].fmt ) )
            GetD3D9Raster( raster )->textureFlags =
                GetD3D9Raster( raster )->textureFlags & 0xF1 | 1;
        raster->cFormat = rasterFmt >> 8;
        return true;
    case rwRASTERTYPECAMERA:
        if ( rasterPixelFmt )
        {
            if ( m_getRasterFormat( m_presentParams.BackBufferFormat ) !=
                 rasterFmt )
                return 0;
        ...
        else
        {
            raster->depth = m_depth;
            raster->cFormat =
                m_getRasterFormat( m_presentParams.BackBufferFormat ) >> 8;
        }
        return true;
    case rwRASTERTYPEZBUFFER:
        if ( rasterPixelFmt )
        {
            ...
            else if ( rasterPixelFmt == rwRASTERFORMAT32 &&
                      m_ZBufferDepth == 32 )
            {
                raster->depth   = 32;
                raster->cFormat = rasterPixelFmt >> 8;
...
bool CRwD3DEngine::m_createZBufferRaster( RwRaster *    raster,
                                          RwD3D9Raster *d3dRaster )
{
    if ( m_ZBufferDepth == 16 )
    {
        d3dRaster->format = m_checkValidZBufferFormat( D3DFMT_D15S1 )
                                ? D3DFMT_D15S1
                                : D3DFMT_D16;
    }
    else if ( m_ZBufferDepth == 32 )
    {
        ...
        else
        {
            if ( m_checkValidZBufferFormat( D3DFMT_D24X4S4 ) )
                ...
                d3dRaster->format = m_checkValidZBufferFormat( D3DFMT_D32 )
                                        ? D3DFMT_D32
                                        : D3DFMT_D24X8;
        }
    }
    ...
    HRESULT hr = pD3DDevice->CreateDepthStencilSurface(
        raster->width, raster->height, d3dRaster->format,
        m_presentParams.MultiSampleType, m_presentParams.MultiSampleQuality, 0,
        &d3dRaster->surface, 0 );
...
bool CRwD3DEngine::m_createTextureRaster( RwRaster *    raster,
                                          RwD3D9Raster *d3dRaster )
{
    ...
    if ( rasterFormat & ( rwRASTERFORMATPAL8 | rwRASTERFORMATPAL4 ) )
    {
        ...
        if ( rasterPixelFmt != rwRASTERFORMAT8888 &&
             rasterPixelFmt != rwRASTERFORMAT888 )
            return false;
        ...
        d3dRaster->format =
            m_rasterConvertTable[( rasterFormat >> 8 ) & 0xF].fmt;
    ...
    {
        if ( pD3DDevice->CreateTexture(
                 raster->width, raster->height, ( ~rasterFormat >> 15 ) & 1,
                 ( d3dRaster->textureFlags & 0xF ) != 0 ? D3DUSAGE_AUTOGENMIPMAP
                                                        : NULL,
                 d3dRaster->format, D3DPOOL_MANAGED, &d3dRaster->texture,
                 nullptr ) < 0 )
            return false;
    }
    return true;
...
{
    return m_d3d9->CheckDeviceFormat( m_adapterIndex, m_deviceType,
                                      m_displayMode.Format, 0, D3DRTYPE_TEXTURE,
                                      fmt ) >= 0;
}
...
bool CRwD3DEngine::m_checkValidZBufferFormat( D3DFORMAT fmt )
{
    return ( m_d3d9->CheckDeviceFormat(
                 m_adapterIndex, m_deviceType, m_displayMode.Format,
                 D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, fmt ) >= 0 ) &&
           ( m_d3d9->CheckDepthStencilMatch( m_adapterIndex, m_deviceType,
                                             m_displayMode.Format,
                                             m_displayMode.Format, fmt ) >= 0 );
}
...
bool CRwD3DEngine::m_checkValidZBufferTextureFormat( D3DFORMAT fmt )
{
    return ( m_d3d9->CheckDeviceFormat(
                 m_adapterIndex, m_deviceType, m_displayMode.Format,
                 D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_TEXTURE, fmt ) >= 0 ) &&
           ( m_d3d9->CheckDepthStencilMatch( m_adapterIndex, m_deviceType,
                                             m_displayMode.Format,
                                             m_displayMode.Format, fmt ) >= 0 );
}
...
bool CRwD3DEngine::m_checkAutoMipmapGenTextureFormat( D3DFORMAT fmt )
{
    if ( m_deviceCaps.Caps2 & D3DCAPS2_CANAUTOGENMIPMAP )
        return m_d3d9->CheckDeviceFormat(
                   m_adapterIndex, m_deviceType, m_displayMode.Format,
                   D3DUSAGE_AUTOGENMIPMAP, D3DRTYPE_TEXTURE, fmt ) == 0;
    return false;
}
...
int CRwD3DEngine::m_getRasterFormat( D3DFORMAT fmt )
{
    ...
    case D3DFMT_P8: return rwRASTERFORMATPAL8;
    case D3DFMT_L8: return rwRASTERFORMATLUM8;
    case D3DFMT_D16_LOCKABLE:
    case D3DFMT_D15S1:
    case D3DFMT_D16: return rwRASTERFORMAT16;
    case D3DFMT_D32:
    case D3DFMT_D24S8:
    case D3DFMT_D24X8:
    case D3DFMT_D24X4S4:
    case D3DFMT_D32F_LOCKABLE:
    case D3DFMT_D24FS8: return rwRASTERFORMAT32;
    default: break;
    }
...
bool CRwD3DEngine::Start()
{
    ...
    if ( ( hr = m_d3d9->CreateDevice( m_adapterIndex, m_deviceType,
                                      m_windowHandle, BehaviorFlags,
                                      &m_presentParams, &pD3DDevice ) ) < 0 )
    {
...
bool CRwD3DEngine::GetModeInfo( RwVideoMode &videoMode, int n )
{
    ...
    {
    case D3DFMT_A8R8G8B8:
    case D3DFMT_A2R10G10B10: videoMode.format = 0x500; break;
    case D3DFMT_X8R8G8B8: videoMode.format = 0x600; break;
    case D3DFMT_R5G6B5: videoMode.format = 0x200; break;
    case D3DFMT_A1R5G5B5: videoMode.format = 0x100; break;
    case D3DFMT_X1R5G5B5: videoMode.format = 0xA00; break;
    default: videoMode.format = 0; break;
    }
    return true;
}
...
bool CRwD3DEngine::Standards( int *fnPtrArray, int arraySize )
{
    ...
    fnPtrArray[1] =
        (int)(void *&)pBeginUpdate; // 0x7F8F20;//CameraBeginUpdate !!must do!!
    fnPtrArray[2] = 0x7FEE20;       // RGBToPixel not used in menu
    fnPtrArray[3] = 0x7FF070;       // PixelToRGB not used in menu
    fnPtrArray[4] =
        (int)(void *&)pRasterCreate; // 0x4CCE60;//RasterCreate !!must do!!
    fnPtrArray[5] =
        (int)(void *&)pRasterDestroy; // 0x4CBB00;//RasterDestroy !!must do!!
    ...
    fnPtrArray[10] =
        (int)(void *&)pEndUpdate; // 0x7F98D0;//CameraEndUpdate !!must do!!
    ...
    fnPtrArray[15] =
        (int)(void *&)pRasterLock; // 0x4C9F90;//RasterLock !!must do!!
    fnPtrArray[16] =
        (int)(void *&)pRasterUnlock; // 0x4CA290;//RasterUnlock !!must do!!
    ...
    fnPtrArray[20] = (int)(void *&)
        pRasterShowRaster; // 0x7F99B0;//RasterShowRaster !!must do!!
    ...
    fnPtrArray[26] = (int)(void *&)
        pNativeTextureRead;    // 0x4CD820;//NativeTextureRead !!must do!!
    fnPtrArray[27] = 0x4CD4D0; // NativeTextureWrite not used in menu
    fnPtrArray[28] =
        (int)(void *&)pTest; // 0x4CBCB0;//RasterGetMipLevels not used in menu
    return true;
}
...
{
    maxTexSize =
        max( m_deviceCaps.MaxTextureWidth, m_deviceCaps.MaxTextureHeight );
    return true;
}
