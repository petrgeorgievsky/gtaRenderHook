...
HRESULT CD3D1XShader::CompileShaderFromFile(
    std::string szFileName, std::string szEntryPoint, std::string szShaderModel,
    ID3DBlob **ppBlobOut, CD3D1XShaderDefineList *localShaderDefineList ) const
{
    auto hr = S_OK;
    ...
    {
        defines.push_back(
            {define.m_sName.c_str(), define.m_sDefinition.c_str()} );
    }
    defines.push_back( {} );
    ID3DBlob *pErrorBlob = nullptr;

    auto stemp = std::wstring( szFileName.begin(), szFileName.end() );
    hr         = D3DCompileFromFile( stemp.c_str(), defines.data(),
    ...
    if ( FAILED( hr ) )
    {
        if ( pErrorBlob )
        {
            g_pDebug->printMsg( reinterpret_cast<const char *>(
...
CD3D1XVertexShader::CD3D1XVertexShader( std::string fileName,
                                        std::string entryPoint )
{
    // Compile shader
    std::string shaderModel =
        "vs_" + g_pStateMgr->GetShaderModel( GET_D3D_FEATURE_LVL );

    if ( FAILED( CompileShaderFromFile( fileName, entryPoint, shaderModel,
                                        &m_pBlob ) ) )
        g_pDebug->printError( "The FX file cannot be compiled." );

    auto pd3DDevice = GET_D3D_DEVICE;
    // Create shader
    if ( FAILED( pd3DDevice->CreateVertexShader(
             m_pBlob->GetBufferPointer(), m_pBlob->GetBufferSize(), nullptr,
             (ID3D11VertexShader **)&m_pShaderDC ) ) )
    {
        m_pBlob->Release();
        m_pBlob = nullptr;
...
void CD3D1XVertexShader::Set()
{
    ...
    pImmediateContext->VSSetShader(
        static_cast<ID3D11VertexShader *>( m_pShaderDC ), nullptr, 0 );
...
CD3D1XPixelShader::CD3D1XPixelShader(
    std::string fileName, std::string entryPoint,
    CD3D1XShaderDefineList *localShaderDefineList )
{
    // Compile shader
    ...
    if ( !CALL_D3D_API(
             CompileShaderFromFile( m_sFilePath, m_sEntryPoint, m_sShaderModel,
                                    &m_pBlob, localShaderDefineList ),
             "Pixel shader compilation error:" + fileName + "/" + entryPoint ) )
        return;

    // Create shader resource
    if ( !CALL_D3D_API(
             GET_D3D_DEVICE->CreatePixelShader(
                 m_pBlob->GetBufferPointer(), m_pBlob->GetBufferSize(), nullptr,
...
#endif // USE_SHADER_CACHING
    CD3D1XShader::Set();
    GET_D3D_CONTEXT->PSSetShader(
        static_cast<ID3D11PixelShader *>( m_pShaderDC ), nullptr, 0 );
#ifdef USE_SHADER_CACHING
    m_pLastShader = this;
#endif // USE_SHADER_CACHING
}
...
void CD3D1XPixelShader::Reload( CD3D1XShaderDefineList *localShaderDefineList )
{
    ...
    // recompile shader
    if ( !CALL_D3D_API_SILENT(
             CompileShaderFromFile( m_sFilePath, m_sEntryPoint, m_sShaderModel,
                                    &pBlob, localShaderDefineList ),
             "Pixel shader compilation error:" + m_sFilePath + "/" +
                 m_sEntryPoint ) )
        return;
    // create shader resource
    if ( !CALL_D3D_API_SILENT( GET_D3D_DEVICE->CreatePixelShader(
...
CD3D1XComputeShader::CD3D1XComputeShader( std::string fileName,
                                          std::string entryPoint )
{ // Compile shader
    std::string shaderModel =
        "cs_" + g_pStateMgr->GetShaderModel( GET_D3D_FEATURE_LVL );
    ...
    auto pd3dDevice = GET_D3D_DEVICE;
    // Create shader
    if ( FAILED( pd3dDevice->CreateComputeShader(
             m_pBlob->GetBufferPointer(), m_pBlob->GetBufferSize(), nullptr,
             reinterpret_cast<ID3D11ComputeShader **>( &m_pShaderDC ) ) ) )
    {
        m_pBlob->Release();
        m_pBlob = nullptr;
...
void CD3D1XComputeShader::Set()
{
    ...
    CD3D1XShader::Set();
    auto pImmediateContext = GET_D3D_CONTEXT;
    pImmediateContext->CSSetShader(
        static_cast<ID3D11ComputeShader *>( m_pShaderDC ), nullptr, 0 );
#ifdef USE_SHADER_CACHING
    m_pLastShader = this;
#endif // USE_SHADER_CACHING
}
...
CD3D1XGeometryShader::CD3D1XGeometryShader( std::string fileName,
                                            std::string entryPoint )
{ // Compile shader
    std::string shaderModel =
        "gs_" + g_pStateMgr->GetShaderModel( GET_D3D_FEATURE_LVL );
    ...
    auto pd3dDevice = GET_D3D_DEVICE;
    // Create shader
    if ( FAILED( pd3dDevice->CreateGeometryShader(
             m_pBlob->GetBufferPointer(), m_pBlob->GetBufferSize(), nullptr,
             reinterpret_cast<ID3D11GeometryShader **>( &m_pShaderDC ) ) ) )
    {
        m_pBlob->Release();
        m_pBlob = nullptr;
...
void CD3D1XGeometryShader::Set()
{
    ...
    pImmediateContext->GSSetShader(
        static_cast<ID3D11GeometryShader *>( m_pShaderDC ), nullptr, 0 );
#ifdef USE_SHADER_CACHING
    m_pLastShader = this;
#endif // USE_SHADER_CACHING
}
...
CD3D1XHullShader::CD3D1XHullShader( std::string fileName,
                                    std::string entryPoint )
{ // Compile shader
    std::string shaderModel =
        "hs_" + g_pStateMgr->GetShaderModel( GET_D3D_FEATURE_LVL );
    ...
    auto pd3dDevice = GET_D3D_DEVICE;
    // Create shader
    if ( FAILED( pd3dDevice->CreateHullShader(
             m_pBlob->GetBufferPointer(), m_pBlob->GetBufferSize(), nullptr,
             reinterpret_cast<ID3D11HullShader **>( &m_pShaderDC ) ) ) )
    {
        m_pBlob->Release();
        m_pBlob = nullptr;
...
void CD3D1XHullShader::Set()
{
    ...
    CD3D1XShader::Set();
    auto pImmediateContext = GET_D3D_CONTEXT;
    pImmediateContext->HSSetShader(
        static_cast<ID3D11HullShader *>( m_pShaderDC ), nullptr, 0 );
#ifdef USE_SHADER_CACHING
    m_pLastShader = this;
#endif // USE_SHADER_CACHING
}
...
CD3D1XDomainShader::CD3D1XDomainShader( std::string fileName,
                                        std::string entryPoint )
{ // Compile shader
    std::string shaderModel =
        "ds_" + g_pStateMgr->GetShaderModel( GET_D3D_FEATURE_LVL );
    ...
    auto pd3dDevice = GET_D3D_DEVICE;
    // Create shader
    if ( FAILED( pd3dDevice->CreateDomainShader(
             m_pBlob->GetBufferPointer(), m_pBlob->GetBufferSize(), nullptr,
             reinterpret_cast<ID3D11DomainShader **>( &m_pShaderDC ) ) ) )
    {
        m_pBlob->Release();
        m_pBlob = nullptr;
...
void CD3D1XDomainShader::Set()
{
    ...
    CD3D1XShader::Set();
    auto pImmediateContext = GET_D3D_CONTEXT;
    pImmediateContext->DSSetShader(
        static_cast<ID3D11DomainShader *>( m_pShaderDC ), nullptr, 0 );
#ifdef USE_SHADER_CACHING
    m_pLastShader = this;
#endif // USE_SHADER_CACHING
}
