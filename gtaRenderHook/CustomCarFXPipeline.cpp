...
CRenderMeshPool<AlphaMesh> CCustomCarFXPipeline::m_aAlphaMeshList( 1000 );
...
void CCustomCarFXPipeline::RenderAlphaList()
{
    ...
    m_aAlphaMeshList.ExecuteForEach( [this]( const AlphaMesh &mesh ) {
        ...
        if ( colorHEX == 0xC8FF00 || colorHEX == 0xFF00FF ||
             colorHEX == 0xFFFF00 || colorHEX == 0xAF00FF )
        {
            paintColor.blue  = 0;
            paintColor.green = 0;
            paintColor.red   = 0;
        ...
        // g_pRenderBuffersMgr->UpdateMaterialDiffuseColor(paintColor);
        float fShininess =
            1.0f - RpMaterialGetFxEnvShininess( curmesh.material );
        float fSpec =
            CCustomCarEnvMapPipeline__GetFxSpecSpecularity( curmesh.material );
        float fMetal = curmesh.material->surfaceProps.specular;
        // RwMatrix *ltm =
        // RwFrameGetLTM((RwFrame*)((RpAtomic*)mesh->entryptr->owner)->object.object.parent);
        g_pRenderBuffersMgr->UpdateWorldMatrix( mesh.worldMatrix );
        g_pRenderBuffersMgr->SetMatrixBuffer();
        g_pRenderBuffersMgr->UpdateMaterialSpecularInt(
            isnan( fSpec ) ? 0.0f : fSpec );
        ...
        if ( curmesh.material->texture &&
             curmesh.material->texture->raster != nullptr )
...
void CCustomCarFXPipeline::Render( RwResEntry * repEntry, void * object, RwUInt8 type, RwUInt32 flags )
{
    ...
    for ( size_t i = 0; i < static_cast<size_t>( entryData->header.numMeshes ); i++ )
    {
        ...
        else
        {
            ...
            {
                m_aAlphaMeshList.Push( { entryData, RwFrameGetLTM( static_cast<RwFrame*>( atomic->object.object.parent ) ), (int)i } );
                continue;
            }
        }
        // Draw mesh
