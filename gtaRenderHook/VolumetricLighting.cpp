...
CD3D1XConstantBuffer<CBVolumetricLighting>
    *CVolumetricLighting::m_pVolumeLightingCB  = nullptr;
...
{
    m_pVolumetricSunlightPS = new CD3D1XPixelShader(
        "shaders/VolumetricLighting.hlsl", "VolumetricSunlightPS",
        &gVolumetricLightingSettings.m_ShaderDefineList );
    m_pVolumetricCombinePS = new CD3D1XPixelShader(
        "shaders/VolumetricLighting.hlsl", "VolumetricCombinePS",
        &gVolumetricLightingSettings.m_ShaderDefineList );
    float renderingScale =
        gVolumetricLightingSettings.GetFloat( "VolumetricRenderingScale" );
    m_pVolumeLightingRaster =
    ...
    gVolumetricLightingSettings.m_aShaderPointers.push_back(
        m_pVolumetricSunlightPS );
    gVolumetricLightingSettings.m_aShaderPointers.push_back(
        m_pVolumetricCombinePS );
    gDebugSettings.DebugRenderTargetList.push_back( m_pVolumeLightingRaster );

    m_pVolumeLightingCB->data.RaymarchingDistance =
        gVolumetricLightingSettings.GetFloat( "RaymarchingDistance" );
    m_pVolumeLightingCB->data.SunlightBlendOffset =
        gVolumetricLightingSettings.GetFloat( "SunlightBlendOffset" );
    m_pVolumeLightingCB->data.SunlightIntensity =
        gVolumetricLightingSettings.GetFloat( "SunlightIntensity" );
...
void CVolumetricLighting::RenderVolumetricEffects( RwRaster *normalsDepth,
                                                   RwRaster *cascadeShadowMap,
                                                   RwRaster *from,
                                                   RwRaster *to )
{
    m_pVolumeLightingCB->data.RaymarchingDistance =
...
void CVolumetricLighting::QueueTextureReload()
{
    ...
    if ( m_bRequiresReloading || dxEngine->m_bScreenSizeChanged )
    {
        float renderingScale =
            gVolumetricLightingSettings.GetFloat( "VolumetricRenderingScale" );
        m_pVolumeLightingRaster->width =
            (int)( RsGlobal.maximumWidth * renderingScale );
        m_pVolumeLightingRaster->height =
...
void VolumetricLightingSettingsBlock::Load( const tinyxml2::XMLDocument &doc )
{
    ...
    m_ShaderDefineList.AddDefine(
        "SUNLIGHT_RM_STEPS",
        std::to_string( gVolumetricLightingSettings.GetUInt(
            "SunlightRaymarchingSteps" ) ) );
}
...
void TW_CALL ReloadVolumetricShadersCallBack( void *value )
{
    ...
    gVolumetricLightingSettings.m_ShaderDefineList.AddDefine(
        "SUNLIGHT_RM_STEPS",
        std::to_string( gVolumetricLightingSettings.GetUInt(
            "SunlightRaymarchingSteps" ) ) );
}
...
void VolumetricLightingSettingsBlock::InitGUI( TwBar *mainBar )
{
    ...
    TwAddButton( mainBar, "Reload volume shaders",
                 ReloadVolumetricShadersCallBack, nullptr,
                 "group=VolumetricLighting" );
    TwAddButton( mainBar, "Reload volume textures",
                 ReloadVolumetricTexturesCallBack, nullptr,
                 "group=VolumetricLighting" );
}
