...
void CVulkanRenderer::m_initRenderPass()
{
	...
	vkCreateRenderPass(m_pDevice->getDevice(), &render_pass_create_info, nullptr, &m_renderPass);
}
...
void CVulkanRenderer::BeginScene()
{
	...
	vkAcquireNextImageKHR(m_pDevice->getDevice(), m_pSwapChain->getSwapChain(), UINT64_MAX, m_imageAquiredSemaphore, VK_NULL_HANDLE,
		&m_currentBuffer);
	m_pCommandBufferMgr->BeginRenderBuffer(m_pSwapChain->getImageList()[m_currentBuffer]);
}
...
{
	m_pCommandBufferMgr->EndRenderBuffer(m_pSwapChain->getImageList()[m_currentBuffer]);

	VkPipelineStageFlags wait_dst_stage_mask = VK_PIPELINE_STAGE_TRANSFER_BIT;

	VkFenceCreateInfo fenceCreateInfo = {};
	fenceCreateInfo.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
	vkCreateFence(m_pDevice->getDevice(), &fenceCreateInfo, NULL, &m_renderFence);
	...
	vkQueueSubmit(m_pDevice->getGraphicsQueue(), 1, &submitInfo, m_renderFence);
	vkWaitForFences(m_pDevice->getDevice(), 1, &m_renderFence, VK_TRUE, UINT64_MAX);
...
void CVulkanRenderer::m_initInstance()
{
	...
	if (vkCreateInstance(&instanceCreateInfo, nullptr, &m_Instance) != VK_SUCCESS)
		g_pDebug->printError("Fuck shit fuck!");
	// GPU Enumeration
	{
		uint32_t gpuCount;
	...
	// Vulkan Instance Layer Enumeration
	{
		...
		vkEnumerateInstanceLayerProperties(&layerPropCount, layerPropList.data());
	}
	// Vulkan Device Layer Enumeration
	{
		uint32_t layerPropCount;
		vkEnumerateDeviceLayerProperties(m_GPU_list[m_GPU_ID],&layerPropCount, nullptr);
		std::vector<VkLayerProperties> layerPropList(layerPropCount);
		vkEnumerateDeviceLayerProperties(m_GPU_list[m_GPU_ID] ,&layerPropCount, layerPropList.data());
	}
}
...
void CVulkanRenderer::m_initDebugCB()
{
	...
	debugCBinfo.flags = /*VK_DEBUG_REPORT_INFORMATION_BIT_EXT | */ VK_DEBUG_REPORT_WARNING_BIT_EXT |
		VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT | VK_DEBUG_REPORT_ERROR_BIT_EXT /*|	VK_DEBUG_REPORT_DEBUG_BIT_EXT*/;

	CreateDebugReportCallbackEXT(m_Instance, &debugCBinfo, nullptr, &m_debugCB);
}
...
void CVulkanRenderer::m_initWindowSurface(HWND& window)
{
	...
	vkCreateWin32SurfaceKHR(m_Instance, &surfaceCreateInfo, nullptr, &m_windowSurface);
}
...
void CVulkanRenderer::m_initSemaphores()
{
	...
	vkCreateSemaphore(m_pDevice->getDevice(), &imageAcquiredSemaphoreCreateInfo, NULL, &m_imageAquiredSemaphore);
	vkCreateSemaphore(m_pDevice->getDevice(), &imageAcquiredSemaphoreCreateInfo, NULL, &m_renderFinishSemaphore);
}
...
VKAPI_ATTR VkBool32 VKAPI_CALL RwVKDebugCB(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
{
