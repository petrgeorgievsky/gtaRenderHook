...
void MeshGetNumVerticesMinIndex( const RxVertexIndex *indices, uint32_t size,
                                 RwUInt32 &numVertices, RwUInt32 &min )
{
    if ( size > 0 )
    {
...
RwTexture *_RwTextureRead( const RwChar *name, const RwChar *maskName )
{
    ...
    std::transform( tex_name.begin(), tex_name.end(), tex_name_lc.begin(),
                    ::tolower );
    g_materialBuffer[tex_name] = {};
    return g_ideTextureList[tex_name] ? g_ideTextureList[tex_name]
                                      : g_ideTextureList[tex_name_lc];
}
                      ...
                      unsigned int triangleCount, bool isTriStrip )
{
    ...
    {
        RwReal length = sqrt( verticles[i].nx * verticles[i].nx +
...
RwResEntry *RHInstanceAtomic( RpAtomic *atomic, void *owner,
                              ...
                              const RpMeshHeader *meshHeader )
{
    ...
    RHEngine::RHPrimitiveType primType =
        RHEngine::RHPrimitiveType::TriangleStrip;
    ...
    RxVertexIndex *indexBuffer =
        new RxVertexIndex[meshHeader->totalIndicesInMesh * 3];
    ...
    }
    {
        ...
        RHEngine::g_pRHRenderer->GetGPUAllocator()->AllocateIndexBuffer(
            bufferInfo, reinterpret_cast<void *&>( d3dIndexBuffer ) );
    }
    delete[] indexBuffer;

    // Vertex data
    RHVertexDescPosColorUVNormals *vertexData =
        new RHVertexDescPosColorUVNormals[atomic->geometry->numVertices];
    ...
    uint32_t v_id = 0;
    for ( ; vertexPos != atomic->geometry->morphTarget[0].verts +
    ...
    GenerateNormals( vertexData, atomic->geometry->numVertices,
                     atomic->geometry->triangles,
                     ...
                     primType == RHEngine::RHPrimitiveType::TriangleStrip );
    {
        ...
        RHEngine::g_pRHRenderer->GetGPUAllocator()->AllocateVertexBuffer(
            vertexBufferInfo, reinterpret_cast<void *&>( d3dVertexBuffer ) );
    }

    delete[] vertexData;

    resEntry->modelInstance =
        new D3D11PrimitiveBatch( meshHeader->serialNum, d3dIndexBuffer,
                                 d3dVertexBuffer, primType, splitData );
...
RenderStatus RwRHInstanceAtomic( RpAtomic *atomic )
{
    ...
    if ( !( rpGEOMETRYNATIVE & geomFlags ) )
    {
        ...
        if ( resEntry == nullptr )
        {
             ...
             */
            resEntry = RHInstanceAtomic( atomic, owner, geometry->repEntry,
                                         meshHeader );
            if ( resEntry == nullptr )
                return RenderStatus::Failure;
...
void SimpleSample::RenderUI()
{
    ...
    if ( ImGui::CollapsingHeader( "Models info" ) )
    {
        ...
        {
            if ( m_ideModelList[it.modelId].dffname.compare(
                     0, strlen( m_sModelFilter ), m_sModelFilter ) != 0 )
                continue;
            ImGui::PushID( id );
            // ImGui::BeginChild(  );
            if ( ImGui::CollapsingHeader(
                     m_ideModelList[it.modelId].dffname.c_str() ) )
            {
                bool selected = it.selected;
                ImGui::Checkbox( "Highlight", &selected );
                it.selected = selected;

                ImGui::Text( "DrawDist:%f",
                             m_ideModelList[it.modelId].drawDist );
                ImGui::Text( "Matrix:\t m00:%f\tm01 : %f\tm02 : %f"
                             "\n\t\t\tm10:%f\tm11:%f\tm12:%f"
                             "\n\t\t\tm20:%f\tm21:%f\tm22:%f"
                             "\n\t\t\tm30:%f\tm31:%f\tm32:%f",
                             it.transform.r[0].vector4_f32[0],
                             it.transform.r[0].vector4_f32[1],
                             it.transform.r[0].vector4_f32[2],
                             it.transform.r[1].vector4_f32[0],
                             it.transform.r[1].vector4_f32[1],
                             it.transform.r[1].vector4_f32[2],
                             it.transform.r[2].vector4_f32[0],
                             it.transform.r[2].vector4_f32[1],
                             it.transform.r[2].vector4_f32[2],
                             it.transform.r[3].vector4_f32[0],
                             it.transform.r[3].vector4_f32[1],
                             it.transform.r[3].vector4_f32[2] );
                float jump_dist = m_ideModelList[it.modelId].drawDist;
                if ( ImGui::Button( "Jump to" ) )
                {
                    m_pMainCameraFrame->ltm.pos = {
                        -m_pMainCameraFrame->ltm.at.x * jump_dist +
                            it.transform.r[3].vector4_f32[0],
                        -m_pMainCameraFrame->ltm.at.y * jump_dist +
                            it.transform.r[3].vector4_f32[1],
                        -m_pMainCameraFrame->ltm.at.z * jump_dist +
                            it.transform.r[3].vector4_f32[2] };
                }
    ...
    if ( ImGui::CollapsingHeader( "Materials info" ) )
    {
        ...
        {
            if ( it.first.compare( 0, strlen( m_sTexFilter ), m_sTexFilter ) !=
                 0 )
                continue;
            ImGui::PushID( id );
            if ( ImGui::CollapsingHeader( it.first.c_str() ) )
            {
                ...
                ImGui::DragFloat( "Roughness", &it.second.Roughness, 0.01f,
                                  0.0f, 1.0f );
                ImGui::DragFloat( "Metallness", &it.second.Metallness, 0.01f,
                                  0.0f, 1.0f );
            }
            ImGui::PopID();
            id++;
...
void SimpleSample::LoadIPL( const RHEngine::String &path, int32_t version )
{
    ...
    while ( memcmp( line, "end", 3 ) != 0 )
    {
        ...
        if ( version == 0 )
        {
            sscanf( line, "%u %s %f %f %f %f %f %f %f %f %f %f", &model_id,
                    dff_name, &pos.x, &pos.y, &pos.z, &scale.x, &scale.y,
                    &scale.z, &quatRot.x, &quatRot.y, &quatRot.z, &quatRot.w );
        }
        else if ( version == 1 )
        {
            sscanf( line, "%u %s %u %f %f %f %f %f %f %f %f %f %f", &model_id,
        ...
        {
            sscanf( line, "%u %s %u %f %f %f %f %f %f %f %u", &model_id,
        ...
        if ( m_ideModelList.find( model_id ) != m_ideModelList.end() )
        {
            ...
            {
                if ( std::find( g_ideLoadedTextureList.begin(),
                                g_ideLoadedTextureList.end(),
                                m_ideModelList[model_id].txdname ) ==
                     g_ideLoadedTextureList.end() )
                {
                    RwTexDictionary *texDict = RH_RWAPI::GTAReadTexDict(
                        textures_path +
                        std::string( m_ideModelList[model_id].txdname ) +
                        ".txd" );
                    ...
                    {
                        result =
                            rwLLLinkGetData( cur, RwTexture, lInDictionary );

                        g_ideTextureList.insert(
                            std::pair<std::string, RwTexture *>(
                                std::string( result->name ), result ) );

                        cur = rwLLLinkGetNext( cur );
                    }
                    g_ideLoadedTextureList.push_back(
                        std::string( m_ideModelList[model_id].txdname ) );
                }
                else
                ...
                if ( std::experimental::filesystem::exists(
                         models_path +
                         std::string( m_ideModelList[model_id].dffname ) +
                         ".dff" ) )
                {
                    RpClump *res;
                    RH_RWAPI::LoadClump(
    ...
    for ( auto [id, clump] : m_ideModelList )
    {
        ...
        if ( clump.object )
        {
            ...
            while ( cur != end )
            {
                ...
                clump.boundSphereRad =
                    max( clump.boundSphereRad, atomic->boundingSphere.radius );
                rwResEnty_ *resEntry = (rwResEnty_ *)atomic->geometry->repEntry;
                auto        splitData = resEntry->modelInstance->SplitData();
                clump.hasAlpha        = std::any_of(
                    splitData.begin(), splitData.end(),
                    []( const RHEngine::MeshSplitData &mesh_data ) {
                        if ( mesh_data.material )
                        {
                            if ( mesh_data.material->texture )
                            {
                                ...
                                {
                                    return ( mesh_data.material->texture->raster
                                                 ->cFormat &
                                             rwRASTERFORMAT8888 ) != 0 ||
                                           ( mesh_data.material->texture->raster
                                                 ->cFormat &
                                             rwRASTERFORMAT1555 ) != 0 ||
                                           ( mesh_data.material->texture->raster
                                                 ->cFormat &
                                             rwRASTERFORMAT4444 ) != 0;
                                }
                            }
                        }
                        return false;
...
void SimpleSample::DrawClump( RHEngine::IRenderingContext *context,
                              RpClump *clump, bool selected,
                              DirectX::XMMATRIX transf_mult )
{
    RwLLLink *cur, *end, *next;
    ...
    while ( cur != end )
    {
        ...
        if ( resEntry != nullptr )
        {
            ...
            const RwMatrix *ltm = RH_RWAPI::_RwFrameGetLTM(
                (RwFrame *)rwObjectGetParent( atomic ) );
            ...
            context->UpdateBuffer( (void *)mPerModelConstantBuffer,
                                   &objTransformMatrix,
                                   sizeof( objTransformMatrix ) );
            context->BindConstantBuffers(
                RHEngine::RHShaderStage::Vertex |
                    RHEngine::RHShaderStage::Pixel,
                { { 0, (void *)mBaseConstantBuffer },
                  { 1, (void *)mPerModelConstantBuffer } } );
            m_pForwardPBRPipeline->DrawObject( context, resEntry->modelInstance,
                                               { selected } );
...
void SimpleSample::RenderWorker( std::uint32_t id )
{
    while ( true )
    {
        ...
        void *frameBufferInternal =
            RH_RWAPI::GetInternalRaster( m_pMainCamera->frameBuffer );
        mDeferredContextList[id]->BindImageBuffers(
            RHImageBindType::RenderTarget, { { 0, frameBufferInternal } } );

        if ( m_pMainCamera->zBuffer )
        {
            void *zBufferInternal =
                RH_RWAPI::GetInternalRaster( m_pMainCamera->zBuffer );
            if ( zBufferInternal )
            {
                mDeferredContextList[id]->BindImageBuffers(
                    RHImageBindType::DepthStencilTarget,
                    { { 0, zBufferInternal } } );
            }
        }
        ...
        mDeferredContextList[id]->BindConstantBuffers(
            RHEngine::RHShaderStage::Vertex | RHEngine::RHShaderStage::Pixel,
            { { 0, (void *)mBaseConstantBuffer },
              { 1, (void *)mPerModelConstantBuffer } } );
        RHEngine::D3D11RenderingContext *d3d_ctxt =
            (RHEngine::D3D11RenderingContext *)mDeferredContextList[id];
        d3d_ctxt->GetStateCache()->GetBlendStateCache()->SetBlendEnable( true,
                                                                         0 );
        for ( uint32_t i = m_batches[id].start; i < m_batches[id].end; i++ )
        {
            DrawClump( mDeferredContextList[id],
                       m_vCurrentFrameRenderList[i].data.object,
                       m_vCurrentFrameRenderList[i].info.selected,
        ...
        m_fRenderingThreadTime[id] = duration_cast<duration<float, std::milli>>(
                                         high_resolution_clock::now() - timept )
                                         .count();
...
void SimpleSample::CustomRender()
{
    ...
    RHEngine::g_pRWRenderEngine->CameraClear(
        m_pMainCamera, &clearColor, rwCAMERACLEARIMAGE | rwCAMERACLEARZ );

    IRenderingContext *context =
        (IRenderingContext *)g_pRHRenderer->GetCurrentContext();
    context->UpdateBuffer( (void *)mBaseConstantBuffer,
                           &RHEngine::g_cameraContext,
                           sizeof( RHEngine::g_cameraContext ) );
    using namespace std::chrono;
    ...
    for ( auto it = m_iplModelInfoMap.begin(); it != m_iplModelInfoMap.end();
          it++ )
    {
        ...
        toCam = {
            ...
            m_pMainCameraFrame->ltm.pos.z - it->transform.r[3].vector4_f32[2],
            0 };

        const float camDist = DirectX::XMVector3Length( toCam ).vector4_f32[0];
        float       minDrawDist = m_bUseConstDrawDist
                                      ? m_fMaxDrawDist
                                      : m_ideModelList[it->modelId].drawDist;

        const float drawDist =
            minDrawDist + m_ideModelList[it->modelId].boundSphereRad;

        if ( camDist < drawDist && m_ideModelList[it->modelId].object )
        {
            // if( m_ideModelList[it->modelId].hasAlpha )
            //    m_vCurrentFrameAlphaRenderList.push_back( {
            //    m_ideModelList[it->modelId], *it } );
            // else
            m_vCurrentFrameRenderList.push_back(
                { m_ideModelList[it->modelId], *it } );
        }
    }
    m_nVisibleObjects = m_vCurrentFrameRenderList.size();
    std::sort( m_vCurrentFrameRenderList.begin(),
               m_vCurrentFrameRenderList.end(),
               []( const RenderingData &a, const RenderingData &b ) {
                   return a.data.drawDist < b.data.drawDist;
               } );
    m_fCullingTime = duration_cast<duration<float, std::milli>>(
                         high_resolution_clock::now() - t1 )
                         .count();
    /*std::sort( m_vCurrentFrameAlphaRenderList.begin(),
       m_vCurrentFrameAlphaRenderList.end(), []( const RenderingData & a, const
       RenderingData & b )
               {
                   return a.data.drawDist > b.data.drawDist;
               } );*/
    t1 = high_resolution_clock::now();
    uint32_t per_thread_req =
        max( m_vCurrentFrameRenderList.size() / max( m_nWorkerThreadCount, 1 ),
             m_nMinMeshPerThread );
    ...
    if ( m_vCurrentFrameRenderList.size() >= per_thread_req )
    {
        ...
        {
            if ( per_thread_req * ( k + 1 ) >=
                 m_vCurrentFrameRenderList.size() )
            {
    ...
    for ( uint32_t i = 0;
          i < min( per_thread_req, m_vCurrentFrameRenderList.size() ); i++ )
    {
        DrawClump( context, m_vCurrentFrameRenderList[i].data.object,
                   m_vCurrentFrameRenderList[i].info.selected,
                   m_vCurrentFrameRenderList[i].info.transform );
    }
    if ( event_wait_list.size() > 0 )
        WaitForMultipleObjects( event_wait_list.size(), event_wait_list.data(),
                                TRUE, INFINITE );
    ...
    m_fRenderingTime = duration_cast<duration<float, std::milli>>(
                           high_resolution_clock::now() - t1 )
                           .count();
...
void SimpleSample::CustomUpdate( float dt )
{
    ...
    DirectX::XMMATRIX rightRotAxis = DirectX::XMMatrixRotationAxis(
        { m_pMainCameraFrame->ltm.right.x, m_pMainCameraFrame->ltm.right.y,
          m_pMainCameraFrame->ltm.right.z },
        ( mousePosRw.y ) );

    DirectX::XMMATRIX upRotAxis = DirectX::XMMatrixRotationAxis(
        { -m_pMainCameraFrame->ltm.up.x, -m_pMainCameraFrame->ltm.up.y,
          -m_pMainCameraFrame->ltm.up.z },
        ( mousePosRw.x ) );
...
bool SimpleSample::CustomInitialize()
{
    ...
    allocator->AllocateConstantBuffer( { sizeof( RHEngine::RHCameraContext ) },
                                       (void *&)mBaseConstantBuffer );
    allocator->AllocateConstantBuffer( { sizeof( DirectX::XMMATRIX ) },
                                       (void *&)mPerModelConstantBuffer );
    m_aCommandListBuffer.resize( m_nMaxThreadCount );
    m_batches.resize( m_nMaxThreadCount );
    m_nCmdListCount = 0;
    for ( uint32_t i = 0; i < m_nMaxThreadCount; i++ )
    {
        m_nBeginRenderingEvents.push_back(
            CreateEvent( NULL, FALSE, FALSE, NULL ) );
        m_nEndRenderingEvents.push_back(
            CreateEvent( NULL, FALSE, FALSE, NULL ) );
        m_fRenderingThreadTime.push_back( 0.0f );
        m_tWorkers.push_back(
            std::move( std::thread( &SimpleSample::RenderWorker, this, i ) ) );
    }
    ...
    for ( uint32_t i = 0; i < m_nMaxThreadCount; i++ )
    {
        ...
        mDeferredContextList.push_back(
            (RHEngine::IRenderingContext *)deferredctxt );
    }
