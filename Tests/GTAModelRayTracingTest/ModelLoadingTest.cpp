...
bool ModelLoadingTest::CustomInitialize()
{
    ...
    desc.bufferFormats.emplace_back( static_cast<uint32_t>( rh::engine::ImageBufferFormat::RGBA8 ) );
    desc.bufferFormats.emplace_back( static_cast<uint32_t>( rh::engine::ImageBufferFormat::RGBA8 ) );
    ...
    m_pLFGIFilterPass = new rw_raytracing_lib::LowFreqGIFilterPass( 1280, 720, 0.33f );
    ...
    for ( auto &p : fs::directory_iterator( "ipls_3_fast" ) ) {
        ...
        if ( file_path.extension() == ".ipl" || file_path.extension() == ".IPL" ) {
            auto clump_list = m_pMapLoader->LoadModels( "models_3",
                                                        m_pMapLoader->LoadIPL( file_path.string(),
                                                                               0 ) );
            m_vClumpList.insert( m_vClumpList.end(), clump_list.begin(), clump_list.end() );
        }
    }
    // load ipl required ids

    allocator->AllocateConstantBuffer( {sizeof( rh::rw::engine::CameraContext )},
                                       mBaseConstantBuffer );
    allocator->AllocateConstantBuffer( {sizeof( DirectX::XMMATRIX ) * 2}, mPerModelConstantBuffer );
...
void ModelLoadingTest::CustomRender()
{
    ...
    rh::rw::engine::RwCameraClear( m_pMainCamera, &clearColor, rwCAMERACLEARIMAGE | rwCAMERACLEARZ );

    context->BindConstantBuffers( rh::engine::ShaderStage::Vertex | rh::engine::ShaderStage::Pixel
    ...
    context->BindImageBuffers( rh::engine::ImageBindType::RenderTarget,
                               {{0,
                                 rh::rw::engine::GetInternalRaster( m_pMainCamera->frameBuffer )}} );
    m_pShadowsPass->Execute( ray_tracer, m_pGBPass->GetGBuffer( 0 ), m_pGBPass->GetGBuffer( 1 ) );
    auto sh_y = m_pGIPass->Execute( ray_tracer,
                                    m_pGBPass->GetGBuffer( 0 ),
                                    m_pGBPass->GetGBuffer( 1 ) );
    auto filtered_gi = m_pLFGIFilterPass->Execute( m_pGBPass->GetGBuffer( 0 ),
                                                   m_pGBPass->GetGBuffer( 1 ),
                                                   sh_y,
                                                   m_pGIPass->GetSHCoCg(),
                                                   m_pGBPass->GetGBuffer( 2 ),
                                                   m_pGBPass->GetGBuffer( 3 ) );
    GenerateQuad( 1280.0f, 720.0f );
    rh::rw::engine::g_pRwRenderEngine->m_p2DRenderer->BindTexture( filtered_gi );
    ...
    rh::rw::engine::g_pRwRenderEngine->RenderStateSet( rwRENDERSTATETEXTURERASTER, 0 );
    rh::rw::engine::g_cameraContext->deltas.z = min( rh::rw::engine::g_cameraContext->deltas.z + 1,
...
void ModelLoadingTest::CustomUpdate( float dt )
{
    ...
    auto ltm = rh::rw::engine::RwFrameGetLTM( m_pMainCameraFrame );
    {
        ...
        DirectX::XMMATRIX rightRotAxis
            = DirectX::XMMatrixRotationAxis( {{{ltm->right.x, ltm->right.y, ltm->right.z}}},
                                             ( mousePosRw.y ) );

        DirectX::XMMATRIX upRotAxis
            = DirectX::XMMatrixRotationAxis( {{{-ltm->up.x, -ltm->up.y, -ltm->up.z}}},
                                             ( mousePosRw.x ) );

        DirectX::XMMATRIX current_rot_xm = DirectX::XMLoadFloat4x4( &currentRot );

        const DirectX::XMMATRIX rotMat = current_rot_xm * rightRotAxis * upRotAxis;
        ltm->right = {rotMat.r[0].vector4_f32[0],
                      rotMat.r[0].vector4_f32[1],
                      rotMat.r[0].vector4_f32[2]};
        ltm->up = {rotMat.r[1].vector4_f32[0],
                   rotMat.r[1].vector4_f32[1],
                   rotMat.r[1].vector4_f32[2]};
        ltm->at = {rotMat.r[2].vector4_f32[0],
...
{
    return DirectX::XMMatrixIdentity() * DirectX::XMMatrixTranslation( 0, x, 0 );
}

void ModelLoadingTest::PrepareDFFs()
{
    m_pMapLoader->BuildScene( rt_scene, ray_tracer, rh::rw::engine::g_cameraContext->viewPos );
}
