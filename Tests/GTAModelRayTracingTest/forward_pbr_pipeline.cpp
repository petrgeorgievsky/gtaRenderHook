...
#include "../GTAModelLoadingTest/forward_pbr_pipeline.h"
...
{
    rh::engine::IGPUAllocator *allocator =
        rh::engine::g_pRHRenderer->GetGPUAllocator();

    allocator->AllocateShader(
        { TEXT( "shaders/d3d11/forward_pbr_pipeline.hlsl" ), TEXT( "BaseVS" ),
          rh::engine::ShaderStage::Vertex },
        mBaseVS );
    allocator->AllocateShader(
        { TEXT( "shaders/d3d11/forward_pbr_pipeline.hlsl" ), TEXT( "NoTexPS" ),
          rh::engine::ShaderStage::Pixel },
        mNoTexPS );
    allocator->AllocateShader(
        { TEXT( "shaders/d3d11/forward_pbr_pipeline.hlsl" ), TEXT( "TexPS" ),
          rh::engine::ShaderStage::Pixel },
        mTexPS );
    allocator->AllocateShader(
        { TEXT( "shaders/d3d11/forward_pbr_pipeline.hlsl" ),
          TEXT( "SelectedPS" ), rh::engine::ShaderStage::Pixel },
        mSelectedPS );

    rh::engine::InputLayoutInfo layout_info;
    layout_info.inputElements = {
        { "POSITION", rh::engine::InputElementType::Vec4fp32 },
...
{
    uint32_t strides[] = {
        sizeof( rh::rw::engine::VertexDescPosColorUVNormals ) };
    uint32_t offsets[] = { 0 };
    ...
    for ( const rh::engine::MeshSplitData &meshData : model->SplitData() )
    {
        ...
        context->FlushCache();
        context->RecordDrawCallIndexed( meshData.numIndex, meshData.startIndex,
                                        0 );
    }
}
