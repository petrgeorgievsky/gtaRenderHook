...
void GTAMapLoader::LoadIDE( const std::string &path )
{
    ...
    while ( memcmp( line, "end", 3 ) != 0 ) {
        ...
        sscanf_s( line, "%u %s %s %u %f", &model_id, dff_name, txd_name, &obj_count, &drawDist );

        if ( mIDEntries.find( model_id ) == mIDEntries.end() && memcmp( dff_name, "LOD", 3 ) != 0
...
std::vector<uint32_t> GTAMapLoader::LoadIPL( const std::string &path, uint8_t version )
{
    ...
    while ( memcmp( line, "end", 3 ) != 0 ) {
        ...
        if ( mIDEntries.find( model_id ) != mIDEntries.end() ) {
            ...
            if ( std::find( ipl_list.begin(), ipl_list.end(), model_id ) == ipl_list.end() )
...
std::vector<uint32_t> GTAMapLoader::LoadBinaryIPL( const std::string &path )
{
    ...
    for ( int i = 0; i < header.item_inst_count; i++ ) {
        ...
        ipl_fstream.read( reinterpret_cast<char *>( &quatRot ), sizeof( quatRot ) );
        ipl_fstream.read( reinterpret_cast<char *>( &model_id ), sizeof( model_id ) );
        ipl_fstream.read( reinterpret_cast<char *>( &interior ), sizeof( interior ) );
        ipl_fstream.read( reinterpret_cast<char *>( &lod_id ), sizeof( lod_id ) );
...
std::vector<RpClump *> GTAMapLoader::LoadModels( const std::string &path, std::vector<uint32_t> ids )
{
    ...
    for ( size_t i = 0; i < ids.size(); i++ ) {
        ...
        if ( !std::filesystem::exists( fp ) )
            continue;
        if ( !rh::rw::engine::LoadClump( mIDEntries[ids[i]].dff_model, fp.generic_string() ) )
                               ...
                               const DirectX::XMFLOAT4 &camPos )
{
    ...
    //        ); end = rw_rh_engine::rwLinkList::GetTerminator( &clump->atomicList
    //        );
    ...
    for ( auto &ipl_entry : mIPLEntries ) {
        ...
        DirectX::XMStoreFloat( &dist_to_cam,
    ...
    for ( auto ipl_entry : mIPLEntries ) {
        if ( mIDEntries[ipl_entry.item_defenition_id].dff_model != nullptr ) {
            ...
            RwLLLink *cur, *end;
            cur = rh::rw::engine::rwLinkList::GetFirstLLLink( &clump->atomicList );
            end = rh::rw::engine::rwLinkList::GetTerminator( &clump->atomicList );
            if ( cur == end )
                continue;

            RpAtomic *atomic = rh::rw::engine::rwLLLink::GetData<RpAtomic>( cur,
                                                                          offsetof( RpAtomic,
                                                                                    inClumpLink ) );
            rh::rw::engine::RwRHInstanceAtomic( atomic, &geometry_interface_36 );

            const RwMatrix *ltm = rh::rw::engine::RwFrameGetLTM(
                static_cast<RwFrame *>( rh::rw::engine::rwObject::GetParent( atomic ) ) );
            geometry_interface_36.Init( atomic->geometry );
            rt_scene->PushModel( ipl_entry.item_defenition_id, &geometry_interface_36 );
            ...
            DirectX::XMMATRIX objTransformMatrix = {ltm->right.x,
                                                    ...
                                                    ltm->pos.z,
                                                    1};
            DirectX::XMMATRIX ws = DirectX::XMLoadFloat4x4( &ipl_entry.world_transform );

            objTransformMatrix = objTransformMatrix * ws;
            DirectX::XMStoreFloat4x4( &blas.world_transform, objTransformMatrix );
    ...
    if ( !blas_inst_list.empty() ) {
        ...
        rw_raytracing_lib::TLAS_BVH tlas = rt_scene->GenerateTLAS( blas_inst_list );
        if ( !tlas.tlas.empty() )
...
{
    rh::engine::IRenderingContext *context = static_cast<rh::engine::IRenderingContext *>(
    ...
    for ( auto ipl_entry : mIPLEntries ) {
        if ( mIDEntries[ipl_entry.item_defenition_id].dff_model != nullptr ) {
            ...
            DirectX::XMStoreFloat4x4( &transf_buf.world_inv,
                                      DirectX::XMMatrixInverse( nullptr, objTransformMatrix ) );

            context->UpdateBuffer( per_obj_cb, &transf_buf, sizeof( transf_buf ) );

            rh::rw::engine::DrawAtomic( atomic,
                                      &geometry_interface_36,
                                      context,
                                      static_cast<rh::engine::IRenderingPipeline *>( pipeline ) );
        }
    }
}
