...
public:
    BVHBuilder();
    BLAS_BVH BuildBVH( RpGeometry *geometry );

    PackedBLAS_BVH PackBLASBVH( const std::vector<BLAS_BVH> &blas );
...
private:
    std::vector<BVHPrimitive> GenerateBVHPrimitiveList( RpGeometry *geometry );
    std::vector<DirectX::XMFLOAT4> debug_line_vertices;
    void *debug_vbuff;
    void *mBaseVS;
    void *mLinePS;
    void *mVertexDecl;
