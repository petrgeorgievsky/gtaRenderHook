...
int32_t RwDefaultRenderingPipeline::AtomicAllInOneNode( RxPipelineNodeInstance * /*self*/,
                                                        const RxPipelineNodeParam *params )
{
    ...
    if ( RwRHInstanceAtomic( atomic, &g_rw35GeometryProxy ) != RenderStatus::Instanced )
        return false;

    // IRenderingContext *context = static_cast<IRenderingContext *>(
    //    g_pRHRenderer->GetCurrentContext() );

    const RwMatrix *ltm = rw_rh_engine::RwFrameGetLTM(
