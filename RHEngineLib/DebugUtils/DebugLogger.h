 ...
 *
 */
enum class LogLevel : uint8_t { ConstrDestrInfo = 0, Info, Warning, Error };
...
public:
   ...
   */
    static void Log( const engine::String &msg, LogLevel logLevel = LogLevel::Info );
...
#define RH_ASSERT( expr ) \
    if ( !( expr ) ) { \
        rh::debug::DebugLogger::Error( TEXT( "Assertion failed: " ) + ToRHString( #expr ) ); \
    }
