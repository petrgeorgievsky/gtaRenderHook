...
{
    std::vector<vk::AttachmentReference> vk_attachment_refs;
    vk_attachment_refs.reserve( ref_list.size() );

    std::transform( ref_list.begin(), ref_list.end(),
                    std::back_inserter( vk_attachment_refs ),
    ...
    : m_vkDevice( create_info.mDevice )
{
    ...
    std::transform(
        create_info.mDesc.mAttachments.begin(),
        create_info.mDesc.mAttachments.end(),
        std::back_inserter( vk_attachment_descs ),
        []( const AttachmentDescription &desc ) -> vk::AttachmentDescription {
            return Convert( desc );
        } );

    std::vector<vk::SubpassDescription> subpass_desc_list;
    std::vector<cppVkSubpassDesc>       subpass_desc_list_temp;

    for ( auto subpass_desc : create_info.mDesc.mSubpasses )
    {
