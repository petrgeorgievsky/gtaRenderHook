                                                           ...
                                                           uint32_t *offsets )
{
    ...
    for ( void *buffer_ptr : buffers ) {
        auto *d3d11buf = reinterpret_cast<D3D11Buffer *>( buffer_ptr );
...
{
    auto *d3d11buf = reinterpret_cast<D3D11Buffer *>( buffer );
...
{
    auto *shader_interface = reinterpret_cast<D3D11Shader *>( shader );
    ...
    uint8_t bindStages, const std::vector<IndexPtrPair> &buffers )
{
    ...
    for ( const IndexPtrPair &pair : buffers ) {
        auto *d3d11buf = reinterpret_cast<D3D11Buffer *>( pair.ptr );
