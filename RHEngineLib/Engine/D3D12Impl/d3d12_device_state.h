...
public:
    ...
    bool GetDisplayModeCount( unsigned int outputId, unsigned int &count ) override;
    ...
    IDeviceOutputView *CreateDeviceOutputView( HWND window, const OutputInfo &info ) override;
    //void InitAllocator( D3D11GPUAllocator &allocator, D3D11DeviceOutputView *output );
...
private:
    /// Device driver type - represents where to do the graphics work on GPU or on
    /// CPU(with different implementations)
