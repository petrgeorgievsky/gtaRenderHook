...
public:
    ...
    virtual bool BindConstantBuffers( uint8_t bindStages, const std::vector<IndexPtrPair> &buffers )
        = 0;
    virtual void BindSamplers( const std::vector<IndexPtrPair> &sampler_states, ShaderStage stage )
        = 0;
    virtual void UpdateBuffer( void *buffer, const void *data, int32_t size ) = 0;
    virtual void SetPrimitiveTopology( PrimitiveType type ) = 0;

    virtual void DispatchThreads( uint32_t x, uint32_t y, uint32_t z ) = 0;

    virtual void RecordDrawCall( uint32_t vertexCount, uint32_t startVertex ) = 0;
    virtual void RecordDrawCallIndexed( uint32_t indexCount,
                                        uint32_t startIndex,
                                        uint32_t baseVertex )
        = 0;
    virtual void CopyImageBuffer( void *dest_buffer,
                                  void *src_buffer,
