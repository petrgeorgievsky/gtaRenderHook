...
bool rh::engine::VulkanRenderer::InitDevice()
{
    ...
    try
    {
        m_vkGraphicsQueueCommandPool =
            m_vkDevice.createCommandPool( cmdPoolInfo );
        ...
        try
        {
            m_vkMainCommandBufferList =
                m_vkDevice.allocateCommandBuffers( mainCmdBufferInfo );
            ...
            try
            {
                ...
                try
                {
                    m_vkMainCmdBufferSemaphore =
                        m_vkDevice.createSemaphore( {} );
                    ...
                    for ( size_t i = 0; i < m_vkMainCmdBufferFences.size();
                          i++ )
                    {
                        try
                        {
                            m_vkMainCmdBufferFences[i] =
                                m_vkDevice.createFence( renderFenceInfo );
                        }
                        catch ( ... )
                        {
                            for ( size_t j = i + 1; j > 0; j-- )
                                m_vkDevice.destroyFence(
                                    m_vkMainCmdBufferFences[i - 1] );
                            m_vkDevice.destroySemaphore(
                                m_vkMainCmdBufferSemaphore );
                            throw;
                        }
                    }
                    ...
                    swapChainInfo.imageFormat =
                        m_aWindowSurfaceFormats[0].format;
                    swapChainInfo.imageColorSpace =
                        m_aWindowSurfaceFormats[0].colorSpace;
                    swapChainInfo.imageExtent =
                        m_vkWindowSurfaceCap.currentExtent;
                    swapChainInfo.imageArrayLayers = 1;
                    swapChainInfo.imageUsage =
                        vk::ImageUsageFlagBits::eColorAttachment |
                        ( m_vkWindowSurfaceCap.supportedUsageFlags &
                          vk::ImageUsageFlagBits::eTransferDst );
                    swapChainInfo.presentMode = vk::PresentModeKHR::
                        eMailbox; // m_aWindowSurfacePresentModes[2];
                    swapChainInfo.imageSharingMode =
                        vk::SharingMode::eExclusive;
                    swapChainInfo
                        .compositeAlpha = vk::CompositeAlphaFlagBitsKHR::
                    ...
                    {
                        m_vkSwapChain =
                            m_vkDevice.createSwapchainKHR( swapChainInfo );
                    }
                    catch ( ... )
                    {
                        for ( auto m_vkMainCmdBufferFence :
                              m_vkMainCmdBufferFences )
...
bool rh::engine::VulkanRenderer::GetAdapterInfo( unsigned int        n,
...
bool rh::engine::VulkanRenderer::SetCurrentAdapter( unsigned int n )
{
    ...
    VkBool32 surfaceSupported = m_aAdapters[n].getSurfaceSupportKHR(
        m_aGraphicsQueueFamilies[0], m_vkWindowSurface );

    if ( !surfaceSupported )
        return false;

    m_vkWindowSurfaceCap =
        m_aAdapters[n].getSurfaceCapabilitiesKHR( m_vkWindowSurface );
    m_aWindowSurfaceFormats =
        m_aAdapters[n].getSurfaceFormatsKHR( m_vkWindowSurface );
    m_aWindowSurfacePresentModes =
        m_aAdapters[n].getSurfacePresentModesKHR( m_vkWindowSurface );

    return true;
}

bool rh::engine::VulkanRenderer::GetOutputCount( unsigned int adapterId,
                                                 int &        c )
{
...
bool rh::engine::VulkanRenderer::GetOutputInfo( unsigned int  n,
                                                std::wstring &info )
{
...
bool rh::engine::VulkanRenderer::GetDisplayModeCount( unsigned int /*outputId*/,
                                                      int &c )
{
...
bool rh::engine::VulkanRenderer::GetDisplayModeInfo( unsigned int /*id*/,
...
bool rh::engine::VulkanRenderer::Present( void *image )
{
    ...
    if ( m_vkGraphicsQueue.presentKHR( presentInfo ) ==
         vk::Result::eErrorOutOfDateKHR )
    {
        rh::debug::DebugLogger::Error( "UNHANDLED OUTOFDATEKHR" );
    }

    m_vkDevice.waitForFences(
        { m_vkMainCmdBufferFences[m_nCurrentMainCmdBuffer] }, true,
        UINT64_MAX );
    m_vkDevice.resetFences(
        { m_vkMainCmdBufferFences[m_nCurrentMainCmdBuffer] } );
...
void rh::engine::VulkanRenderer::BindViewPorts(
    const std::vector<ViewPort> & /*viewports*/ )
{
}

void *
rh::engine::VulkanRenderer::AllocateImageBuffer( const ImageBufferInfo &info )
{
    switch ( info.type )
    {
    case rh::engine::ImageBufferType::Unknown: break;
    case rh::engine::ImageBufferType::BackBuffer:
        return new VulkanBackBuffer( m_vkDevice, m_vkSwapChain );
    case rh::engine::ImageBufferType::TextureBuffer: break;
    case rh::engine::ImageBufferType::DepthBuffer: break;
    case rh::engine::ImageBufferType::RenderTargetBuffer:
    case rh::engine::ImageBufferType::DynamicTextureArrayBuffer: break;
    }
    return nullptr;
}

bool rh::engine::VulkanRenderer::FreeImageBuffer( void *          buffer,
...
bool rh::engine::VulkanRenderer::BindImageBuffers(
    ImageBindType /*bindType*/, const std::vector<IndexPtrPair> &buffers )
{
    ...
    m_vkMainCommandBufferList[m_nCurrentMainCmdBuffer].pipelineBarrier(
...
bool rh::engine::VulkanRenderer::ClearImageBuffer(
    ImageClearType clearType, void *buffer,
    const std::array<float, 4> &clearColor )
{
    ...
    case rh::engine::ImageClearType::Color:
        ...
        m_vkMainCommandBufferList[m_nCurrentMainCmdBuffer].clearColorImage(
            backBuffer->GetImage(), vk::ImageLayout::eTransferDstOptimal,
            vkClearColor, { subresRange } );

        return true;
    case rh::engine::ImageClearType::Depth: break;
    case rh::engine::ImageClearType::Stencil: break;
    case rh::engine::ImageClearType::DepthStencil: break;
    }
    return false;
}
...
bool rh::engine::VulkanRenderer::EndCommandList( void *cmdList )
{
    if ( cmdList == nullptr )
    {
        ...
        m_vkGraphicsQueue.submit(
            { submitInfo }, m_vkMainCmdBufferFences[m_nCurrentMainCmdBuffer] );
...
bool rh::engine::VulkanRenderer::RequestSwapChainImage( void *frameBuffer )
{
    ...
    currentBackBufferId =
        m_vkDevice
            .acquireNextImageKHR( m_vkSwapChain, UINT64_MAX,
                                  m_vkSwapChainImageSemaphore, nullptr )
            .value;
