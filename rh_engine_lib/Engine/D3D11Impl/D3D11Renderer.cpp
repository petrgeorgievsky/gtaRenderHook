...
bool rh::engine::D3D11Renderer::InitDevice()
{
    ...
    {
        rh::debug::DebugLogger::Error(
            "Failed to retrieve current display mode info!" );
        return false;
    }
    ...
    m_pDeviceState.InitRenderingContext( *m_mainRenderingContext,
                                         m_gpuMemoryAllocator, false );

    m_pOutputView->Resize( m_gpuMemoryAllocator, displ_info.height,
                           displ_info.width );
...
void *
rh::engine::D3D11Renderer::AllocateImageBuffer( const ImageBufferInfo &info )
{
...
bool rh::engine::D3D11Renderer::FreeImageBuffer( void *          buffer,
                                                 ImageBufferType type )
{
    return m_gpuMemoryAllocator->FreeImageBuffer( buffer, type );
}

bool rh::engine::D3D11Renderer::BindImageBuffers(
...
bool rh::engine::D3D11Renderer::ClearImageBuffer(
    ImageClearType clearType, void *buffer,
    const std::array<float, 4> &clearColor )
{
    return m_mainRenderingContext->ClearImageBuffer( clearType, buffer,
                                                     clearColor );
}
...
void rh::engine::D3D11Renderer::BindViewPorts(
...
void rh::engine::D3D11Renderer::ImGUIStartFrame() { ImGui_ImplDX11_NewFrame(); }
