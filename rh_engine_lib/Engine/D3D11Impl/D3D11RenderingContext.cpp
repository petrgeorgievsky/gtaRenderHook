...
void D3D11RenderingContext::Init( ID3D11DeviceContext *context,
...
bool rh::engine::D3D11RenderingContext::ClearImageBuffer(
...
void rh::engine::D3D11RenderingContext::RecordDrawCall( uint32_t vertexCount,
...
{
    m_renderStateCache.GetSamplerStateCache()->SetSamplerState( sampler_states,
                                                                stage );
}

void rh::engine::D3D11RenderingContext::SetPrimitiveTopology(
    PrimitiveType type )
{
    switch ( type )
    {
    case PrimitiveType::Unknown: break;
    case PrimitiveType::LineList:
        m_pContextImpl->IASetPrimitiveTopology(
...
void rh::engine::D3D11RenderingContext::BindVertexBuffers(
    const std::vector<void *> &buffers, uint32_t *strides, uint32_t *offsets )
{
    ...
    for ( void *buffer_ptr : buffers )
    {
        ...
        internal_buffers.push_back( d3d11buf ? d3d11buf->GetBuffer()
                                             : nullptr );
    }

    m_pContextImpl->IASetVertexBuffers(
        0, static_cast<UINT>( internal_buffers.size() ),
        internal_buffers.data(), strides, offsets );
}
...
bool rh::engine::D3D11RenderingContext::BindIndexBuffer( void *buffer )
{
    ...
    m_pContextImpl->IASetIndexBuffer(
        d3d11buf ? d3d11buf->GetBuffer() : nullptr, DXGI_FORMAT_R16_UINT, 0 );
    return true;
}
...
void rh::engine::D3D11RenderingContext::UpdateBuffer( void *      buffer,
                                                      const void *data,
                                                      int32_t     size )
{
    auto *d3d11buf = reinterpret_cast<D3D11BufferOld *>( buffer );
...
rh::engine::D3D11RenderStateCache *
rh::engine::D3D11RenderingContext::GetStateCache()
{
...
void rh::engine::D3D11RenderingContext::DispatchThreads( uint32_t x, uint32_t y,
                                                         uint32_t z )
{
    ...
    uint8_t bindStages, const std::vector<IndexPtrPair> &buffers )
{
    ...
    if ( bindStages & ShaderStage::Compute )
        m_pContextImpl->CSSetConstantBuffers( min_slot, size,
                                              internal_buffers.data() );
    if ( bindStages & ShaderStage::Domain )
        m_pContextImpl->DSSetConstantBuffers( min_slot, size,
                                              internal_buffers.data() );
    if ( bindStages & ShaderStage::Geometry )
        m_pContextImpl->GSSetConstantBuffers( min_slot, size,
                                              internal_buffers.data() );
    if ( bindStages & ShaderStage::Hull )
        m_pContextImpl->HSSetConstantBuffers( min_slot, size,
                                              internal_buffers.data() );
    if ( bindStages & ShaderStage::Pixel )
        m_pContextImpl->PSSetConstantBuffers( min_slot, size,
                                              internal_buffers.data() );
    if ( bindStages & ShaderStage::Vertex )
        m_pContextImpl->VSSetConstantBuffers( min_slot, size,
                                              internal_buffers.data() );
    return false;
}

void rh::engine::D3D11RenderingContext::RecordDrawCallIndexed(
    uint32_t indexCount, uint32_t startIndex, uint32_t baseVertex )
{
    m_pContextImpl->DrawIndexed( indexCount, startIndex,
                                 static_cast<int>( baseVertex ) );
}

void rh::engine::D3D11RenderingContext::CopyImageBuffer(
    void *dest_buffer, void *src_buffer, uint32_t dest_subres_id,
    uint32_t dest_pos_x, uint32_t dest_pos_y, uint32_t dest_pos_z )
{
    auto dest_res = static_cast<IGPUResource *>( dest_buffer );
    auto src_res  = static_cast<IGPUResource *>( src_buffer );
    m_pContextImpl->CopySubresourceRegion(
        static_cast<ID3D11Resource *>( dest_res->GetImplResource() ),
        dest_subres_id, dest_pos_x, dest_pos_y, dest_pos_z,
        static_cast<ID3D11Resource *>( src_res->GetImplResource() ), 0,
        nullptr );
}
