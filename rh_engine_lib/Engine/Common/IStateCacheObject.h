 ...
 * will override previous state, and spend lots of time doing some API stuff
 * twice, which is indeed big CPU overhead and should be avoided
 */
class IStateCacheObject
{
public:
    IStateCacheObject() = default;
    virtual ~IStateCacheObject() noexcept = default;
    IStateCacheObject( const IStateCacheObject & ) = delete;
