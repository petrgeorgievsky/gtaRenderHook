...
uint32_t rh::rw::engine::RwStreamRead( void *stream, void *buffer,
                                       uint32_t length )
{
...
bool rh::rw::engine::RwStreamFindChunk( void *stream, uint32_t type,
                                        uint32_t *lengthOut,
                                        uint32_t *versionOut )
{
    uint32_t readType;
    ...
    while ( _rwStreamReadChunkHeader( stream_, &readType, &readLength,
                                      &readVersion, nullptr ) )
    {
        if ( readType == type )
        {
...
static char *StringStreamRead( char *nativeString, void *stream,
                               uint32_t length )
{
    ...
    {
        nativeString = rh::rw::engine::hAllocArray<char>(
            "StreamString", static_cast<size_t>( length ) );
        if ( !nativeString )
        {
            return nullptr;
        }
    ...
    while ( length > 0 )
    {
        ...
        if ( rh::rw::engine::RwStreamRead( stream, multiByteString,
...
bool rh::rw::engine::_rwStreamReadChunkHeader( std::istream *stream,
