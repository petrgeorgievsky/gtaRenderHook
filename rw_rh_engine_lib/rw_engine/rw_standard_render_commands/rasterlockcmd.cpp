...
RwRasterLockCmd::RwRasterLockCmd( RwRaster *raster, int32_t accessMode )
    : m_pRaster( raster )
    , m_nAccessMode( accessMode )
{}

bool RwRasterLockCmd::Execute( void *&res_data_ptr )
{
    /* Prepare lock info */
    auto mipLevel = static_cast<RwUInt8>( ( m_nAccessMode & 0xFF00 ) >> 8 );
    ...
    m_pRaster->cpPixels = static_cast<RwUInt8 *>(
        malloc( static_cast<size_t>( m_pRaster->width * m_pRaster->height * 4 ) ) );
